...
const Step2: React.FC<StepProps> = ({ formik }) => {
  ...
  useEffect(() => {
    if (isInitialMount.current) {
      // Populate the initial descriptions only on first render
      serviceTypeOptions.forEach((option) => {
        if (initialDescriptions[option] && !formik.values.serviceDescriptions[option]) {
...
export const MultiStepForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchServiceData = async () => {
      if (isEditMode) {
        try {
          const response = await axios.get(`/api/1241029013026-service/${params.serviceName}`);
  ...
  const handleSubmit = async (values: FormData, { setSubmitting, resetForm }: { setSubmitting: (isSubmitting: boolean) => void, resetForm: () => void }) => {
    ...
    try {
      let response;
      if (isEditMode) {
        response = await axios.put(`/api/1241029013026-service/${params.serviceName}`, values, {
          headers: {
